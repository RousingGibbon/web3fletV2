...
BRIDGE_TOKEN = '0x0000000000000000000000000000000000000000'  # Адрес моста, если есть
...
class WooFiSwap:
    def __init__(self, private_key: str, from_network: NetworkInfo, to_network: NetworkInfo):
        ...
        self.cross_swap_router = from_network.web3.eth.contract(
            ...
            abi=json.load(open('abis/cross_abi.json'))
        )
        self.on_chain_swap_router = from_network.web3.eth.contract(
            address=from_network.on_chain_router_address,
            abi=json.load(open('abis/woofi_abi.json'))
        )  # Замените на фактический адрес контракта и ABI для on-chain обмена, если требуется

    def _execute_transaction(self, txn: dict) -> Optional[dict]:
        try:
    ...
    def cross_chain_swap(self, token_from: str, token_to: str, amount: float, slippage: float) -> Optional[dict]:
        try:
            # Получение и вывод информации о балансе и цене газа
            ...
            txn = self.cross_swap_router.functions.crossSwap(
                ...
                },
                {
                    ...
                    'data': b''  # или другой байтовый массив данных
                },
                {
    ...
    def on_chain_swap(self, token_from: str, token_to: str, amount: float, slippage: float) -> Optional[dict]:
        ...
        try:
            ...
            txn = self.on_chain_swap_router.functions.swap(
                ...
                amount_in,
                amount_out_min,
                self.account.address,  # Куда отправить токены
                self.account.address  # Откуда отправить токены (в данном случае тот же адрес)
            ).build_transaction({
                'chainId': self.from_network.chain_id,
                'gas': 2000000,
                'gasPrice': gas_price,
            ...
            if balance < max_gas_cost:
                raise ValueError(
                    f"Insufficient funds for gas: have {self.from_network.web3.from_wei(balance, 'ether')} ETH, need {self.from_network.web3.from_wei(max_gas_cost, 'ether')} ETH")
